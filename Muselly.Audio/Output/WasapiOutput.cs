...
using W = Muselly.Audio.Output.WasapiInterop;

namespace Muselly.Audio.Output;

/// <summary>
/// Windows audio output via WASAPI (shared mode, event-driven) on the default render endpoint. All COM
/// lives on a dedicated MTA render thread: it activates the endpoint, initialises at the device mix format
/// (32-bit float interleaved), then fills the shared buffer each time WASAPI signals its event.
/// </summary>
...
internal sealed class WasapiOutput : IAudioOutput
{
    ...
    public void Start(AudioRenderCallback callback)
    {
        lock (_lock)
        {
            ...
            _ready.Reset();
            _running = true;
            _thread = new Thread(RenderThread) { IsBackground = true, Priority = ThreadPriority.Highest, Name = "muselly-wasapi" };
            _thread.Start();
            _ready.Wait(3000);
    ...
    private unsafe void RenderThread()
    {
        ...
        try
        {
            ...
            Check(devEnum.GetDefaultAudioEndpoint(W.eRender, W.eConsole, out devPtr), "GetDefaultAudioEndpoint");
            ...
            var rc = (W.IAudioRenderClient)rcObj;

namespace Muselly.Audio.Decoding;

/// <summary>
/// Decoded PCM audio held in memory: interleaved 32-bit float samples at the file's own sample rate and
/// channel count. The playback engine resamples these to the device rate as it plays.
/// </summary>
public sealed class AudioSampleBuffer
{
    public AudioSampleBuffer(float[] samples, int channels, int sampleRate)
    {
        Samples = samples;
        Channels = channels < 1 ? 1 : channels;
        SampleRate = sampleRate <= 0 ? 44100 : sampleRate;
    }

    /// <summary>Interleaved samples (frame f, channel c is at f*Channels + c).</summary>
    public float[] Samples { get; }

    public int Channels { get; }

    public int SampleRate { get; }

    /// <summary>Number of frames (samples per channel).</summary>
    public long FrameCount => Samples.Length / Channels;
}
